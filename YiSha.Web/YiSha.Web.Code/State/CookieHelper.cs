...
namespace YiSha.Web.Code.State
{
    public static class CookieHelper
    {
        ...
        /// 写cookie值
        /// </summary>
        public static void WriteCookie(string sName, string sValue, bool httpOnly = true)
        {
            var hca = GlobalContext.ServiceProvider?.GetService<IHttpContextAccessor>();
            var option = new CookieOptions { Expires = DateTime.Now.AddDays(30), HttpOnly = httpOnly };
        ...
        public static void WriteCookie(string sName, string sValue, int expires, bool httpOnly = true)
        {
            var hca = GlobalContext.ServiceProvider?.GetService<IHttpContextAccessor>();
            var option = new CookieOptions { Expires = DateTime.Now.AddMinutes(expires), HttpOnly = httpOnly };
            hca?.HttpContext?.Response.Cookies.Append(sName, sValue, option);
        }
